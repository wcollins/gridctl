...
func TestValidate_Agent(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			stack:   base([]Agent{{Image: "alpine"}}),
			wantErr: true,
			errMsg:  "agents[0].name",
		},
		{
			name: "duplicate agent names",
			stack: base([]Agent{
				{Name: "a1", Image: "alpine", Uses: []ToolSelector{{Server: "s1"}}},
				{Name: "a1", Image: "nginx", Uses: []ToolSelector{{Server: "s1"}}},
			}),
			wantErr: true,
			errMsg:  "duplicate agent name",
		},
		{
			name: "agent name conflicts with server",
			stack: base([]Agent{
				{Name: "s1", Image: "alpine", Uses: []ToolSelector{{Server: "s1"}}},
			}),
			wantErr: true,
			errMsg:  "conflicts with an MCP server",
		},
		{
			name: "agent name conflicts with resource",
			stack: &Stack{
				Name:       "test",
				Network:    Network{Name: "net"},
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
				Resources:  []Resource{{Name: "db", Image: "postgres:16"}},
				Agents:     []Agent{{Name: "db", Image: "alpine", Uses: []ToolSelector{{Server: "s1"}}}},
			},
			wantErr: true,
			errMsg:  "conflicts with a resource",
		},
		// Headless agent validation
		{
			name: "headless agent runtime + image conflict",
			stack: base([]Agent{
				{Name: "a1", Runtime: "claude-code", Image: "alpine", Prompt: "test", Uses: []ToolSelector{{Server: "s1"}}},
			}),
			wantErr: true,
			errMsg:  "cannot have both 'runtime' and 'image'",
		},
		{
			name: "headless agent runtime + source conflict",
			stack: base([]Agent{
				{Name: "a1", Runtime: "claude-code", Source: &Source{Type: "git", URL: "https://example.com"}, Prompt: "test", Uses: []ToolSelector{{Server: "s1"}}},
			}),
			wantErr: true,
			errMsg:  "cannot have both 'runtime' and 'source'",
		},
		{
			name: "headless agent missing prompt",
			stack: base([]Agent{
				{Name: "a1", Runtime: "claude-code", Uses: []ToolSelector{{Server: "s1"}}},
			}),
			wantErr: true,
			errMsg:  "prompt",
		},
		// Container agent validation
		{
			name: "container agent no image or source",
			stack: base([]Agent{
				{Name: "a1", Uses: []ToolSelector{{Server: "s1"}}},
			}),
			wantErr: true,
			errMsg:  "must have either 'image', 'source', or 'runtime'",
		},
		{
			name: "container agent image + source conflict",
			stack: base([]Agent{
				{Name: "a1", Image: "alpine", Source: &Source{Type: "git", URL: "https://example.com"}, Uses: []ToolSelector{{Server: "s1"}}},
			}),
			wantErr: true,
			errMsg:  "cannot have both 'image' and 'source'",
		},
		// Uses dependencies
		{
			name: "uses referencing unknown server",
			stack: base([]Agent{
				{Name: "a1", Image: "alpine", Uses: []ToolSelector{{Server: "nonexistent"}}},
			}),
			wantErr: true,
			errMsg:  "not found in mcp-servers",
		},
		{
			name: "uses self-reference",
			stack: &Stack{
				Name:       "test",
				Network:    Network{Name: "net"},
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
				Agents: []Agent{
					{
						Name:  "a1",
						Image: "alpine",
						A2A:   &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "skill1", Name: "Skill 1"}}},
						Uses:  []ToolSelector{{Server: "a1"}},
					},
				},
			},
			wantErr: true,
			errMsg:  "cannot reference itself",
		},
		{
			name: "uses referencing non-A2A agent",
			stack: base([]Agent{
				{Name: "a1", Image: "alpine", Uses: []ToolSelector{{Server: "s1"}}},
				{Name: "a2", Image: "alpine", Uses: []ToolSelector{{Server: "a1"}}},
			}),
			wantErr: true,
			errMsg:  "must have A2A enabled",
		},
		// A2A block validation
		{
			name: "A2A missing skill ID",
			stack: base([]Agent{
				{
					Name:  "a1",
					Image: "alpine",
					Uses:  []ToolSelector{{Server: "s1"}},
					A2A:   &A2AConfig{Enabled: true, Skills: []A2ASkill{{Name: "Skill 1"}}},
				},
			}),
			wantErr: true,
			errMsg:  "a2a.skills[0].id",
		},
		{
			name: "A2A missing skill name",
			stack: base([]Agent{
				{
					Name:  "a1",
					Image: "alpine",
					Uses:  []ToolSelector{{Server: "s1"}},
					A2A:   &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "skill1"}}},
				},
			}),
			wantErr: true,
			errMsg:  "a2a.skills[0].name",
		},
		{
			name: "A2A duplicate skill ID",
			stack: base([]Agent{
				{
					Name:  "a1",
					Image: "alpine",
					Uses:  []ToolSelector{{Server: "s1"}},
					A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{
						{ID: "skill1", Name: "Skill 1"},
						{ID: "skill1", Name: "Skill 2"},
					}},
				},
			}),
			wantErr: true,
			errMsg:  "duplicate skill ID",
		},
		// Advanced network mode
		{
			name: "advanced mode missing agent network",
			stack: &Stack{
				Name:       "test",
				Networks:   []Network{{Name: "net1", Driver: "bridge"}},
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000, Network: "net1"}},
				Agents:     []Agent{{Name: "a1", Image: "alpine", Uses: []ToolSelector{{Server: "s1"}}}},
			},
			wantErr: true,
			errMsg:  "required when 'networks' is defined",
		},
		...
		},
		{
			...
			stack: base([]Agent{
				{Name: "a1", Runtime: "claude-code", Prompt: "You are helpful", Uses: []ToolSelector{{Server: "s1"}}},
			}),
		},
	}
...
func TestValidate_A2AAgent(t *testing.T) {
	base := func(a2aAgents []A2AAgent) *Stack {
		return &Stack{
			Name:       "test",
			Network:    Network{Name: "net"},
			MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
	...
	tests := []struct {
		...
		{
			name:    "missing A2A agent name",
			stack:   base([]A2AAgent{{URL: "https://example.com"}}),
			wantErr: true,
			errMsg:  "a2a-agents[0].name",
		},
		{
			name:    "missing A2A agent URL",
			stack:   base([]A2AAgent{{Name: "remote"}}),
			wantErr: true,
			errMsg:  "a2a-agents[0].url",
		},
		{
			name: "duplicate A2A agent names",
			stack: base([]A2AAgent{
				{Name: "remote", URL: "https://a.com"},
				{Name: "remote", URL: "https://b.com"},
			}),
			wantErr: true,
			errMsg:  "duplicate A2A agent name",
		},
		{
			name: "A2A agent name conflicts with local agent",
			stack: &Stack{
				Name:       "test",
				Network:    Network{Name: "net"},
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
				Agents:     []Agent{{Name: "agent1", Image: "alpine", Uses: []ToolSelector{{Server: "s1"}}}},
				A2AAgents:  []A2AAgent{{Name: "agent1", URL: "https://example.com"}},
			},
			wantErr: true,
			errMsg:  "conflicts with a local agent",
		},
		{
			name: "A2A agent name conflicts with server",
			stack: &Stack{
				Name:       "test",
				Network:    Network{Name: "net"},
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
				A2AAgents:  []A2AAgent{{Name: "s1", URL: "https://example.com"}},
			},
			wantErr: true,
			errMsg:  "conflicts with an MCP server",
		},
		{
			name: "invalid auth type",
			stack: base([]A2AAgent{
				{Name: "remote", URL: "https://example.com", Auth: &A2AAuth{Type: "oauth"}},
			}),
			wantErr: true,
			errMsg:  "auth.type",
		},
		{
			name: "auth type set but missing token_env",
			stack: base([]A2AAgent{
				{Name: "remote", URL: "https://example.com", Auth: &A2AAuth{Type: "bearer"}},
			}),
			wantErr: true,
			errMsg:  "auth.token_env",
		},
		{
			name: "valid A2A agent with bearer auth",
			stack: base([]A2AAgent{
				{Name: "remote", URL: "https://example.com", Auth: &A2AAuth{Type: "bearer", TokenEnv: "TOKEN"}},
			}),
		},
		{
			name: "valid A2A agent without auth",
			stack: base([]A2AAgent{
				{Name: "remote", URL: "https://example.com"},
			}),
		},
		{
			name: "auth type none does not require token_env",
			stack: base([]A2AAgent{
				{Name: "remote", URL: "https://example.com", Auth: &A2AAuth{Type: "none"}},
			}),
		},
	}
...
func TestValidate_AgentCycleDetection(t *testing.T) {
	tests := []struct {
		name    string
		stack   *Stack
		wantErr bool
	...
	}{
		{
			...
			stack: &Stack{
				...
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
				Agents: []Agent{
					{Name: "a1", Image: "alpine", A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "s1", Name: "S1"}}}, Uses: []ToolSelector{{Server: "s1"}}},
					{Name: "a2", Image: "alpine", Uses: []ToolSelector{{Server: "a1"}}},
				},
			},
		},
		{
			...
			stack: &Stack{
				...
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
				Agents: []Agent{
					{Name: "a1", Image: "alpine", A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "s1", Name: "S1"}}}, Uses: []ToolSelector{{Server: "a2"}}},
					{Name: "a2", Image: "alpine", A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "s2", Name: "S2"}}}, Uses: []ToolSelector{{Server: "a1"}}},
				},
			},
			wantErr: true,
		...
		},
		{
			...
			stack: &Stack{
				...
				Agents: []Agent{
					{Name: "a1", Image: "alpine", A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "s1", Name: "S1"}}}, Uses: []ToolSelector{{Server: "a2"}}},
					{Name: "a2", Image: "alpine", A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "s2", Name: "S2"}}}, Uses: []ToolSelector{{Server: "a3"}}},
					{Name: "a3", Image: "alpine", A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "s3", Name: "S3"}}}, Uses: []ToolSelector{{Server: "a1"}}},
				},
			},
			wantErr: true,
			errMsg:  "circular dependency",
		...
		{
			name: "multiple independent chains no cycles",
			stack: &Stack{
				Name:       "test",
				Network:    Network{Name: "net"},
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
				Agents: []Agent{
					{Name: "a1", Image: "alpine", A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "s1", Name: "S1"}}}, Uses: []ToolSelector{{Server: "s1"}}},
					{Name: "a2", Image: "alpine", Uses: []ToolSelector{{Server: "a1"}}},
					{Name: "a3", Image: "alpine", A2A: &A2AConfig{Enabled: true, Skills: []A2ASkill{{ID: "s3", Name: "S3"}}}, Uses: []ToolSelector{{Server: "s1"}}},
					{Name: "a4", Image: "alpine", Uses: []ToolSelector{{Server: "a3"}}},
				},
			},
		},
		{
			name: "server deps ignored in cycle detection",
			stack: &Stack{
				Name:       "test",
				Network:    Network{Name: "net"},
				MCPServers: []MCPServer{{Name: "s1", Image: "alpine", Port: 3000}},
				Agents: []Agent{
					{Name: "a1", Image: "alpine", Uses: []ToolSelector{{Server: "s1"}}},
					{Name: "a2", Image: "alpine", Uses: []ToolSelector{{Server: "s1"}}},
				},
			},
		},
	}
