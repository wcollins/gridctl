// Adapted from Tremor AreaChart [v1.0.0]
// Themed for Obsidian Observatory (always dark, no light mode classes)
// Removed @remixicon/react dependency and legend slider feature
  ...
  return (
    <li
      className={cn(
        ...
        hasOnValueChange ? "cursor-pointer hover:bg-surface-highlight" : "cursor-default",
      )}
      onClick={(e) => {
        e.stopPropagation()
        onClick?.(name, color)
    ...
    >
      ...
      <p
        className={cn(
          "truncate text-xs whitespace-nowrap text-text-secondary",
          hasOnValueChange && "group-hover:text-text-primary",
          activeLegend && activeLegend !== name ? "opacity-40" : "opacity-100",
        )}
      >
        {name}
...
const ChartTooltip = ({ active, payload, label, valueFormatter }: ChartTooltipProps) => {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-md border text-sm shadow-md border-border/50 bg-surface-elevated">
          ...
          {payload.map(({ value, category, color }, index) => (
            <div key={`id-${index}`} className="flex items-center justify-between space-x-8">
              <div className="flex items-center space-x-2">
                ...
                />
                <p className="text-right whitespace-nowrap text-text-secondary">{category}</p>
              </div>
              <p className="text-right font-medium whitespace-nowrap tabular-nums text-text-primary">
  ...
  return (
    <div ref={ref} className={cn("h-80 w-full", className)} {...other}>
      <ResponsiveContainer>
        ...
        >
          {showGridLines ? (
            <CartesianGrid className="stroke-border/40 stroke-1" horizontal={true} vertical={false} />
          ) : null}
          <XAxis
          ...
          >
            {yAxisLabel && (
              <Label
                ...
                angle={-90}
                offset={-15}
                className="fill-text-secondary text-sm font-medium"
              >
                {yAxisLabel}
              </Label>
            )}
            ...
            return (
              <React.Fragment key={category}>
                ...
                <Area
                  ...
                  activeDot={(props: any) => {
                    const { cx: cxCoord, cy: cyCoord, stroke, strokeLinecap, strokeLinejoin, strokeWidth, dataKey } =
                      props
                    return (
                      <Dot
                        className={cn(
                  ...
                  dot={(props: any) => {
                    const { stroke, strokeLinecap, strokeLinejoin, strokeWidth, cx: cxCoord, cy: cyCoord, dataKey, index: dotIndex } =
                      props
                    if (
