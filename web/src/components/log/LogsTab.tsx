...
export function LogsTab() {
  ...
  useEffect(() => {
    if (!hasSource || isPaused || !bottomPanelOpen || bottomPanelTab !== 'logs') {
  ...
  }, [hasSource, isPaused, bottomPanelOpen, bottomPanelTab, fetchLogs]);
      ...
      <div className="flex items-center justify-between px-4 h-9 flex-shrink-0 border-b border-border/30 bg-surface-elevated/20">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-medium text-text-secondary">
            {sourceName ? sourceName : 'No source'}
          </span>
          {isGateway && (
            <span className="text-[9px] px-1.5 py-0.5 bg-primary/10 text-primary rounded font-medium border border-primary/20">
              Structured
            </span>
          )}
          {isPaused && (
          ...
          {!autoScroll && !isPaused && (
            <span className="text-[9px] px-1.5 py-0.5 bg-surface-highlight text-text-muted rounded-full font-medium border border-border/30">
              Scrolled
            </span>
          )}
        </div>
