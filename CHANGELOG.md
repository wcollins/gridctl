...
- `docs/skills.md` lands as the canonical reference for the three skill flavors (prompt / code-ts / code-go) and the hybrid pattern. One prose section per flavor (when to reach for it, what files the scaffold writes, the post-scaffold hint and primary run command), one section on the hybrid pattern with both Go (`ctx.SkillBody()`) and TS (`skill.body`) snippets citing `examples/registry/items/incident-triage-hybrid/`, one section on recursive composability that names the `pkg/agent/sandbox/recursive_test.go` smoke test as the load-bearing invariant, and one operational-sharp-edges section for the Go plugin path (host/plugin Go version match, no unloading, Linux/macOS only, manifest guardrails on `go_version` + `go_mod_hash`, the `RegisterSkill(*skill.Registry) error` plugin contract, one-broken-plugin-does-not-block-start semantics). Closes with an explicit "what we do NOT support, and why" section enumerating the deliberate non-features (`kind:` frontmatter field, sub-process Go fallback, per-skill `allowed_providers`, flavor-specific publish paths, `gridctl run` for prompt-only, Go plugin hot-reload, two parallel `Define` shapes) so future feature requests land against a stated stance rather than absence of one. `docs/README.md` indexes the new guide alongside `scaling.md` and `troubleshooting.md`.
- Three-flavor skill examples land under `examples/registry/items/`: `triage-ts/` (basic typed pattern in TypeScript - uses the new `skill.body` / `skill.name` JS globals to feed the SKILL.md severity matrix into an `llm()` call's `system` slot), `triage-go/` (the Go-flavored counterpart, built as a Go plugin via `gridctl agent build triage-go`), and `incident-triage-hybrid/` (the hybrid pattern in code form - the Go handler reads its own SKILL.md body via `ctx.SkillBody()` and uses it as the `agent.ChatRequest.System` value, so editing the severity matrix or runbook in SKILL.md changes runtime behaviour without touching code). All three examples ship with realistic SRE incident-response prose - severity matrices, decision rules, runbook steps - rather than lorem-ipsum, so a copy-paste user lands with something they can actually adapt. Each Go example exports `RegisterSkill(*skill.Registry) error` and ships a `skill_test.go` whose `TestBuildRequest_*` cases are the load-bearing assertion that `ctx.SkillBody()` carries the prose into `ChatRequest.System` verbatim - the hybrid contract is byte-checked, not narrated.
- agentskills.io compatibility integration test at `tests/integration/anthropic_skill_compat_test.go`. Vendors a real prompt-only skill from `github.com/anthropics/skills` (`skills/brand-guidelines/SKILL.md` plus its Apache-2.0 LICENSE) into `tests/integration/testdata/anthropic_skills/`, drops it verbatim into a temp registry root with the canonical `skills/<name>/SKILL.md` layout, and asserts the gridctl walker round-trips it: `store.Load()` succeeds, `store.GetSkill()` returns the skill, `Description` is populated, `HandlerLanguage` is empty (prompt-only classification), `sk.Body` matches the bytes after the closing `---` verbatim, and `registry.ValidateSkillFull` reports no errors. Runs in CI under `-tags=integration -race` alongside the existing integration suite. Refresh by re-vending from upstream when the agentskills.io spec evolves; the testdata README documents the source paths and licenses.
- Gateway-builder Go-plugin loader. `pkg/controller/gateway_builder.go` constructs a shared `*skill.Registry` at `Build()` time and walks the registry store for skills with `HandlerLanguage == "go"`. For each, `loadGoSkillPlugins` reads the adjacent `dist/manifest.json` and enforces the two guardrails Phase 4 wrote into it: `go_version` against `runtime.Version()` and `go_mod_hash` against a fresh hash of the daemon's resolved `go.mod`. Mismatches (or a missing manifest) emit an actionable `slog.Warn` ("plugin built with go1.26.3, daemon running go1.27.0 - rebuild with `gridctl agent build <name>`") and the plugin is skipped - the opaque `plugin.Open` toolchain-mismatch error never reaches the operator. Missing `go_mod_hash` on either side reads as "skip the check, not the plugin" (Pitfall 9). Only after guardrails pass: `plugin.Open`, `plugin.Lookup("RegisterSkill")`, and a call to the canonical `func(*skill.Registry) error` symbol. After plugin registration, each Definition is re-decorated with the on-disk SKILL.md body via the new `skill.WithSkillBody(ctx, body)` helper so `ctx.SkillBody()` returns the same prose a non-plugin in-process registration would surface - preserving the hybrid pattern across the plugin boundary. Per-skill failures (missing `.so`, `plugin.Open` failure, missing `RegisterSkill` symbol, wrong signature, non-nil error from `RegisterSkill`) are logged at warn and the loop continues; one broken plugin never blocks gateway start. `SetSkillRegistry` is called before `Router().AddClient(registryServer)` so the first `RefreshTools` surfaces Go-skill tools without a second pass. Plugin loading is unavailable on Windows (Go stdlib constraint); the windows build replaces `openAndRegisterGoSkill` with a stub that returns a clear error so the loader logs and skips rather than failing to compile.
- `gridctl agent build <skill>` now compiles Go-handler skills as Go plugins (`go build -buildmode=plugin`) instead of returning a "Phase H deferred" error. Writes `dist/skill.so` and a `dist/manifest.json` with the same TS-path shape (`name`, `description`, `handler: "go"`, `handler_path: "skill.so"`, `source_hash`, `input_schema`) plus two guardrail fields the gateway-builder plugin loader will key against in Phase 5: `go_version` (the building toolchain via `runtime.Version()`) and `go_mod_hash` (sha256 of the `go.mod` resolved by walking up from the handler). Standalone skill scaffolds without a parent `go.mod` log at warn and ship the manifest without `go_mod_hash` rather than failing the build. Windows returns a clear pre-flight error - "go skill build requires Linux or macOS - Go plugins are not available on Windows" - before invoking the toolchain so the operator does not see an opaque `-buildmode=plugin not supported on windows/amd64` toolchain message. `gridctl agent validate <skill>` for Go handlers now parses `skill.go` via `go/parser` and reports a clear error when the plugin entry point `func RegisterSkill(*skill.Registry) error` is missing or has the wrong signature, catching the most common copy-paste mistake before the operator wastes a `go build` round-trip.
- **Breaking**: `skill.Define[I, O]` and `skill.MustDefine[I, O]` now take a fourth argument - the SKILL.md body - and the typed runner's first parameter is `skill.RunContext` instead of `context.Context`. `RunContext` embeds `context.Context` (cancellation and request-scoped values flow through unchanged) and adds `SkillBody() string` plus `SkillName() string` so a Go skill can drive the hybrid pattern: feed the markdown body straight into `llm.Generate`'s `System` slot, key per-skill state on the registered name. Body and name are closure-captured at Define time; reads are a single pointer chase with no per-call I/O. Pass `""` for the body when registering programmatically (tests, hand-built fixtures); `ctx.SkillBody()` reads as the empty string and the hybrid pattern degrades gracefully. Migrated three first-party callers - `pkg/agent/skill/skill_test.go`, `pkg/agent/sandbox/recursive_test.go`, the Go scaffold body in `pkg/agent/dev/scaffold/scaffold.go` - and the scaffold's `skill_test.go` ships a `fakeRunContext` stub satisfying the new interface so authors land with passing tests out of the box.
- TS hybrid parity for the `skill.body` / `skill.name` surface: `pkg/agent/sandbox/sandbox.go` `Bindings` gains `SkillBody string` and `SkillName string`; `installSkillBinding` exposes a `skill` global with `body` and `name` properties (read-only, always defined as strings) - same shape Go's `RunContext` provides. The `@gridctl/agent` require shim re-exports `skill` alongside `tool` / `llm` / `parallel` / `handoff` / `approval`. `makeDispatcherBindings` in `pkg/controller/gateway_builder.go` resolves the body from the registry store per-call so the values match what a Go skill would receive via `ctx.SkillBody()` for the same skill - recursive composability invariant: a TS skill that handoffs to another skill receives that other skill's body, not the caller's.
- `gridctl agent init --lang go` now writes a runnable typed Go skill: `SKILL.md` + `skill.go` + `skill_test.go`. The scaffolded `skill.go` is `package main` (so Phase 4 can compile it via `go build -buildmode=plugin`), declares `HelloInput`/`HelloOutput` structs with `json` + `jsonschema` tags, exposes `New() *skill.Definition` returning a `skill.MustDefine[HelloInput, HelloOutput]` registration, and exports the plugin contract `RegisterSkill(*skill.Registry) error` the gateway-builder loader will call after `plugin.Open` (Phase 5). An empty `func main() {}` stub keeps plain `go build` happy for the compile-check pass; the plugin build ignores it. `pkg/agent/dev/scaffold/scaffold.go` gains `helloSkillGo`/`helloSkillGoTest` templates and exports `HelloSkillGo`/`HelloSkillGoTest` so the regression suite runs scaffold output through `go build` + `go test` verbatim - same parity guarantee `HelloSkillTS` provides for the TS path. New `TestScaffoldGoOutputCompiles` (skipped under `-short`) wires a self-contained module with a local replace directive at the gridctl repo and exercises the toolchain end-to-end.
- `gridctl agent init --lang ts|go` and `gridctl agent init --prompt-only`. The TS branch (default) produces the existing three-file scaffold unchanged; `--prompt-only` writes a single `SKILL.md` (no `skill.ts`, no `skill.go`, no `agent.json`) so authors can hand-edit a prompt body that surfaces to upstream MCP clients verbatim. `--prompt-only` is mutually exclusive with `--lang` - the pre-flight error fires before any files are written. Post-scaffold hint differentiates: TS prints `gridctl agent dev --root <dir>`; prompt-only prints `gridctl skill list --remote && gridctl run <name>`. `pkg/agent/dev/scaffold/scaffold.go` gains `Options.Language` (`""`/`"ts"` → TS default, `"prompt"` → SKILL.md only, `"go"` → Go skill scaffold); `starterFiles` branches on the flavor; new `promptSkillMD(name)` renders the prompt-only body with active state so `store.Load()` picks it up.
...
- Typed Skill SDK at `pkg/agent/skill/`: `skill.Define[I, O](name, description, run)` infers a JSON Schema from the input struct (jsonschema tags via `github.com/invopop/jsonschema`) and returns a `*skill.Definition` whose `Invoker` matches `pkg/mcp.AgentClient.CallTool` exactly, so local execution and remote (over-MCP) execution share one code path. `skill.Registry` exposes registered skills as `[]mcp.Tool` and dispatches `CallTool` by name.
- TS skill sandbox at `pkg/agent/sandbox/`: TypeScript skills (default-export async function) transpile via esbuild (LoaderTS, ES2015 CommonJS) and run on a per-call goja runtime with `tool()`, `llm()`, `parallel()`, `handoff()`, and `approval()` bindings. `tool()` flows through `agent.ToolCaller` so existing tracing, pricing, replica routing, vault auth, and tool whitelisting apply; `handoff()` routes through the same `SkillCaller` interface the registry server uses, so a TS skill handing off to a Go skill or another TS skill traverses the same code path an upstream MCP client would. `approval()` is a Phase C stub that auto-approves; the real CLI/web/MCP gate lands in Phase E.
- Registry walker recognises `skill.go` and `skill.ts` siblings of `SKILL.md` and records the handler language and relative path on `AgentSkill`; `Store.HandlerPath(name)` resolves the absolute path. `pkg/registry.Server.Tools()` and `Server.CallTool()` (previously no-op stubs) now expose typed skills as MCP tools - programmatically registered skills via `Server.SetSkillRegistry`, TS-handler skills via `Server.SetTSDispatcher`. Markdown-only skills continue to surface as MCP prompts unchanged.
- Single-writer multi-agent orchestrator at `pkg/agent/orchestrator/`. `orchestrator.New[State](caller, initial)` returns an `Orchestrator[State]` whose State is mutable only through `Apply(func(*State) error)` and readable only through `Snapshot()` (deep copy via JSON round-trip). `Handoff[State, Out]` and `ParallelHandoff[State, Out]` dispatch subagents through `agent.ToolCaller` - the same surface `*skill.Registry` and the gateway adapter both satisfy, so local typed skills, TS skills via the sandbox dispatcher, and remote MCP servers share one code path. Subagents never receive `*State`; single-writer enforcement is structural. Parallel handoffs are clamped at `HardMaxParallel = 4` (configurable down via `SetMaxParallel`); requests above the ceiling clamp with a logged warning. Each handoff and parallel batch emits an OTel span (`agent.orchestrator.handoff`, `agent.orchestrator.parallel`) under tracer `gridctl.agent.orchestrator`.
- JSONL run persistence and time-travel resume at `pkg/agent/persist/`. `persist.Store` writes typed `Event` records (run_started, run_completed, node_enter, node_exit, tool_call, tool_result, llm_call, llm_chunk, structured_output, approval_request, approval_response, error) to `~/.gridctl/runs/<run_id>.jsonl` as an append-only ledger; reads scan the file line-by-line and ignore trailing partial lines so a crash mid-write cannot corrupt earlier history. `Recorder` serialises concurrent writes from one run with a monotonic sequence counter; reopening a run continues numbering without gaps. `Store.BuildResumePlan(run_id, from_step)` projects the ledger up to a chosen node so the runtime can rehydrate state and continue execution.
- Approval gate primitive at `pkg/agent/compose/`. `compose.NewGate(run_id, recorder, registry, notifier)` returns a `Gate` that satisfies `sandbox.Approver`, so the existing TS `approval(prompt)` binding now persists `approval_request`, fires a `Notifier` (default `SlogNotifier` for the operator audit line; `MultiNotifier` fans out to MCP / web push when wired), blocks on a process-wide `Registry`, and records `approval_response` once a CLI / web / MCP consumer resolves it. Default 24h timeout; warning at 80% of the window; `WithTimeout` / `WithWarnFraction` / `WithTimeoutPolicy` / `WithSkill` / `WithLogger` configure per-call.
- Agent-runtime API surface at `internal/api/agent_runs.go`. `GET /api/agent/runs` lists recorded runs (newest first); `GET /api/agent/runs/{run_id}` returns the typed event timeline; `GET /api/agent/runs/{run_id}/events` streams events as Server-Sent Events; `POST /api/agent/runs/{run_id}/resume` builds a `ResumePlan` from the ledger; `POST /api/agent/runs/{run_id}/approve` resolves the run's pending approval through the in-process registry. The daemon wires a `persist.Store` and `compose.Registry` at apply time via `pkg/controller/gateway_builder.go`.
- `gridctl runs` CLI subcommand for inspecting, resuming, and approving runs. `runs list` (table or `--format json`), `runs inspect <run_id>` (typed timeline), `runs trace <run_id>` (OTel-shaped JSON projection), `runs resume <run_id> [--from-step <node_id>]` (renders the resume plan and pings the daemon), `runs approve <run_id> [--decision approve|reject] [--reason <text>]` (POSTs to `/api/agent/runs/{id}/approve`). All structured-data subcommands honour Article X (`--format json`).
- Approval banner in the web UI (`web/src/components/agent/ApprovalBanner.tsx`). Polls `/api/agent/runs` every 5s, surfaces every run currently `awaiting_approval` as a banner under the header, and offers Approve / Reject buttons that POST to `/api/agent/runs/{run_id}/approve` so the suspended run resumes immediately.
- `gridctl run <skill>` top-level command (Phase G). Resolves a registered typed skill from `~/.gridctl/registry/skills`, executes the TS handler in-process via the gridctl sandbox, persists the run as JSONL at `~/.gridctl/runs/<run_id>.jsonl`, and streams typed events to stdout. `--input '<json>' | @file | -` accepts inline JSON, a file, or stdin; `--format json` emits NDJSON for machine consumers; `--quiet` suppresses the event stream and prints only the final output. Recursive composability: `handoff()` resolves against the local registry so a TS skill calling another TS skill traverses the same dispatcher path the gateway uses. Go-handler skills surface a clear deferred-to-Phase-H error rather than silently failing; `tool()` and `llm()` bindings remain unwired in the standalone CLI path and surface as JS errors at call time so the missing wiring is visible rather than faked.
- `gridctl agent build <skill>` and `gridctl agent validate <skill>` (Phase G). `agent validate` runs `registry.ValidateSkillFull` and, for TS handlers, transpiles the source via the sandbox's TS-aware esbuild path so authors see syntax errors before invocation. `agent build` writes a publishable manifest (`manifest.json` + transpiled `skill.js`) to `<skill_dir>/dist/` (override with `--out`); the manifest carries the source's SHA-256 fingerprint so reruns over identical input produce identical artifacts. Go-handler build is reported as deferred (Phase H wires `SetSkillRegistry` from compiled artifacts). Both subcommands honour Article X (`--format json`). Sandbox transpile path exposed via the new `sandbox.TranspileTS` wrapper so the CLI shares one transpiler with the runtime.
- Visual IDE for the agent runtime (Phase F slices 1–3). New `pkg/agent/dev/` package - `parser/` walks Go AST and TS source (regex/lexer, no TS compiler dependency) to extract a flat node list of recognised primitives (`tool`, `llm`, `parallel`, `handoff`, `approval`); `watcher/` recursively watches `*.go` / `*.ts` / `SKILL.md` / `agent.json` with a 200 ms coalescing debounce; `devserver/` exposes the IDE HTTP surface (`GET /api/agent/dev/skills`, `GET /api/agent/dev/skills/{name}`, `GET /api/agent/dev/events` SSE); `scaffold/` renders the `agent init` starter (SKILL.md + skill.ts + agent.json). New `gridctl agent` Cobra subcommand: `agent init` writes a runnable hello-world TS skill in cwd, `agent dev [--port 8181]` boots the IDE dev server. `internal/api/api.go` exposes `/api/agent/dev/*` routes when the daemon is wired with `SetAgentDevServer`. Frontend lands at the new `/agent` route (`web/src/components/agent/ide/`): three-pane shell (skill sidebar, NodeList or React Flow Canvas, inspector drawer), shared trace overlay (status pills + latency + cost), click-to-`$EDITOR` jumps via `vscode://file/...:line`, and "Resume from here" on completed nodes when a run is active. Code-split as a route-level chunk (26 kB; main bundle stays at 877 kB, under the 900 kB regression budget). Code is canon - the IDE never writes back to source; canvas mutations (Slice 4) deferred.
- Three agent-runtime optimize heuristics (`unbounded_loop`, `oversized_prompt`, `untyped_handoff`) appended to the `optimize.Analyze` chain. `unbounded_loop` (warn) flags any run that re-entered a single node ≥20 times, naming the hot node so the user knows where to add a termination condition or max-iter cap. `oversized_prompt` (warn) flags any run whose largest `llm_call` shipped ≥100k input tokens, naming the model so the user can correlate with context-window pressure. `untyped_handoff` (info) flags runs where ≥50% of child handoffs (with at least 3 children) skipped the typed Skill SDK envelope, surfacing typing-discipline drift. New `optimize.RunStat` and `optimize.Stats.RunStats` carry per-run aggregates so the heuristics stay free of an import on `pkg/agent/persist`. `internal/api/optimize.go` walks the most recent 50 runs in the wired `*persist.Store` and reduces their JSONL ledgers into the new shape; absent stores yield nil so the heuristics skip silently rather than firing on absence of data.
- LLM observability wrapper at `pkg/agent/llm/observed/`. `observed.New(inner, "llm:<provider>", WithAccumulator(acc))` decorates any `agent.ChatModel` with OTel spans (`agent.llm.generate` / `agent.llm.stream` under tracer `gridctl.agent.llm`, carrying `gen_ai.*` attributes), per-call cost via `pricing.CalculateBreakdown`, and `metrics.Accumulator.RecordCost` with the synthetic provider name (no MCP envelope spoofing). Spans propagate from the parent context so they nest under existing `mcp.routing` traces when invoked from a tool-call path. The wrapper is a drop-in replacement for the inner `ChatModel` at the call site; sandbox `llm()` bindings, Go-skill direct calls, and the playground service can adopt it without touching the underlying provider packages.
- Sync `AGENTS.md` with the agent-runtime layout: expand the `pkg/agent/` tree to list the `compose/`, `orchestrator/`, `persist/`, `sandbox/`, `skill/`, and `llm/observed/` subdirectories; add a new "Agent Runtime Architecture" section that names the three-layer mental model (Gateway / Agent Runtime / Skill Registry); replace the YAML-`workflow:` "Skill Workflows" section (removed in PR #581) with a Go/TS skill model that documents `skill.Define[I, O]`, the sandbox bindings (`tool()`, `llm()`, `parallel()`, `handoff()`, `approval()`), the orchestrator surface, JSONL persistence, and the `gridctl run` / `gridctl runs` / `gridctl agent` CLI surface; update the `pkg/registry/` tree to drop the deleted `dag.go`/`template.go`/`executor.go` files and call out `Store.HandlerPath`; refresh the API endpoints listing to include `/api/agent/runs/*`, `/api/agent/dev/*`, and `/api/playground/*`; refresh the `/api/optimize` endpoint description to enumerate every shipped heuristic.
